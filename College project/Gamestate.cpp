...
{
    scorefont.loadFromFile("imageserterte/NineTsuki.otf");
    sf::Texture temp;
    temp.loadFromFile("Resources/Images/Textures/2D-Fantasy-Mummies-Sprite-Sheets7cop.png");
...
void Gamestate::Renderthesnacketable(sf::RenderTarget* target)
{
    ...
    for (int j = 0; j < 30; j++)
    {
        sprite1.setPosition((2180 / 2) - float(i) * 16, float((1350 / 2)) - float(j) * 16); target->draw(sprite1);
    }
    splayingtable.setPosition((2180 / 2) - 39 * 16, float((1350 / 2)) - 29 * 16); target->draw(splayingtable);
...
void Gamestate::Renderthesnackebody(sf::RenderTarget* target)
{
    for (int i = 0; i < num; i++)
    {
        if (i == 0)
        {
            ...
            case UP:
                head.loadFromFile("imageserterte/head_up.png");
                shead.setPosition(float(2180 / 2) - s[i].x * 16, float((1350 / 2)) - s[i].y * 16); target->draw(shead);
                break;
        ...
        else if (i == num - 1)
        {
            ...
            case 0:
                ...
                case 1:
                    tail.loadFromFile("imageserterte/tail_down.png");
                    stail.setPosition(float(2180 / 2) - s[i].x * 16, float(float((1350 / 2))) - s[i].y * 16); target->draw(stail);
                    break;
                case -1:
                    tail.loadFromFile("imageserterte/tail_up.png");
                    stail.setPosition(float(2180 / 2) - s[i].x * 16, float((1350 / 2)) - s[i].y * 16); target->draw(stail);
                    break;
                default:
                    break;
                }
        ...
        else
        {
            ...
            case 0:
                ...
                case 1:
                    ...
                    sbody.setPosition(float(2180 / 2) - s[i].x * 16, float(1350 / 2) - s[i].y * 16); target->draw(sbody);
                    break;
                case -1:
                    body.loadFromFile("imageserterte/body_vertical.png");
                    sbody.setPosition(float(2180 / 2) - s[i].x * 16, float(1350 / 2) - s[i].y * 16); target->draw(sbody);
...
{
        { sprite3.setPosition(float(2180 / 2) - f.x * 16, float(1350 / 2) - f.y * 16); target->draw(sprite3); }
}
...
{
        { sbomb.setPosition(float(2180 / 2) - b[0].x * 16, float(1350 / 2) - b[0].y * 16); target->draw(sbomb); }
     { sbomb.setPosition(float(2180 / 2) - b[1].x * 16, float(1350 / 2) - b[1].y * 16); target->draw(sbomb); }
}
...
if (s[0].x < 0) s[0].x = 39; if (s[0].x > 39) s[0].x = 0;
if (s[0].y < 0) s[0].y = 29; if (s[0].y > 29) s[0].y = 0;
}
...
{
    for(int i=0;i<2;i++)
        if (this->Pmenu[i]->getButtonPresed(0)) 
        {
            this->endState();
        }
        else if (this->Pmenu[i]->getButtonPresed(1)) 
        {
            this->unpausedstate();
            break;
        }
...
void Gamestate::update(const float& dt)
{
    ...
    this->updatepausedkeys(float(pow(10,10))*dt);
    this->updateButtons();
    if (!this->paused &&!this->is_die) {
